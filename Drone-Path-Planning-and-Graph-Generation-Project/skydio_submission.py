...
class GenerateGraph:
  ...
  def _collect_regional_data(self) -> int:
    ...
    for endpoint in self.overpassEndpoints:
      try:
        self.overpass = Overpass(endpoint=endpoint)
        self.result = self.overpass.query(self.query)
        self.count = self.result.countElements()

        if self.debug:
          print("Overpass endpoint -> %s" % endpoint)

        break
      except Exception as err:
        lastException = err
        if self.debug:
          print("Overpass query failed for %s" % endpoint)
    else:
      raise lastException
...
class DrawGraph:
  ...
  def draw_path(self, path : list,
    ...
    pathData = []
    for entry in path:
      pathData.append({"name": entry["name"],
                       "color": hex_to_rgb(entry["color"]),
                       "path": entry["path"]})
